...
!     call SN_Pantheon()
    ...
    initpoint = (/ 0.718573d0, 0.238273d0, 6.54624d0 /)
    priormin = (/ 0.6d0, 0.1d0, 1.0d0 /)
    priormax = (/ 0.8d0, 0.5d0, 20.0d0 /)
    jumpsize = (/ 0.001d0, 0.001d0, 0.01d0 /)
...
function Htaddottc(a, addottc)
    ...
    Htaddottc = (exp(LambertW((2.0d0**n_MEC*a**(3.0d0*n_MEC)* &
    addottc**n_MEC*n_MEC)/ abs(-(a*(ob + oc)) &
    - 2.0d0*Omegar())**n_MEC)/ &
    (2.0d0*n_MEC))*sqrt(a*(ob + oc) + Omegar()))/a**2

end function Htaddottc
...
function addottc()
    ...
    addottc = ((2.0d0**(n_MEC - 1.0d0)*(ob + oc + Omegar())**n_MEC)/ &
    (abs(-ob - oc - 2.0d0*Omegar())**n_MEC* &
    log(1.0d0/sqrt(ob + oc + Omegar()))))**(-1.0d0/n_MEC)

end function addottc
...
function Ht(a)
    ...
    Ht = (exp(LambertW((2.0d0**n_MEC*a**(3.0d0*n_MEC)*n_MEC* &
    addottc()**n_MEC)/ &
    abs(-(a*(ob + oc)) - 2.0d0*Omegar())**n_MEC)/ &
    (2.0d0*n_MEC))*sqrt(a*(ob + oc) + Omegar()))/a**2
...
function faddott(a)
    ...
    faddott = ((n_MEC*addottc()**n_MEC)/ &
    LambertW((2.0d0**n_MEC*n_MEC*addottc()**n_MEC)/ &
    abs((-(a*ob) - a*oc - 2.0d0*Omegar())/a**3)**n_MEC))**(1.0d0/n_MEC) &
    - addottc()

end function faddott
...
function ac()
    ...
    ac = bisection(faddott, 0.001d0, 1.0d0, 1.0d-4)

end function ac

function LambertW(x) result(w)
    implicit none
    real(8), intent(in) :: x
    real(8) :: w, w_old, tol
    integer :: i, maxit
    ...
    ! Initial guess: good choice is log(1+x)
    w = log(1.0d0 + x)

    do i = 1, maxit
    w_old = w
    ! Newton-Raphson step:
    w = w - (w*exp(w) - x) / (exp(w)*(w + 1.0d0))
    if (abs(w - w_old) < tol*abs(w)) exit
    end do
