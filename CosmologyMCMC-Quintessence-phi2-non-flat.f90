...
!     call SN_Pantheon()
    ...
    initpoint = (/ 0.68642d0, 0.0477214d0, 0.253357d0, 0.003d0, 1.0d0 /)
    jumpsize = (/ 0.001d0, 0.001d0, 0.001d0, 0.001d0, 0.001d0 /)
    priormin = (/ 0.6d0, 0.01d0, 0.1d0, -0.1d0, 0.16d0 /)
    priormax = (/ 0.8d0, 0.1d0, 0.5d0, 0.1d0, 3.0d0 /)
    ...
    do i = 1, max_iter
        ...
        if (isnan(chi2params_new)) then
            chi2params_new = 1.0d4
        end if

        points_local(rank + 1, i, :) = params_new

        write(10 + rank, "(12e25.16)") 1.0d0, chi2params_new, params_new, &
        H0(), om(), ol(), age(), Vt0
...
! function of the Hubble parameter for the LCDM model
...
! Compute the background dynamics for the quintessence dark energy model
...
! store background
! write(12, "(4e25.16)") arraya(i), arrayHt(i), arrayphit(i), arrayphitp(i)
