...
program CosmologyMCMC
    ...
    real(8), parameter :: cH0 = 2997.92458d0
    ...
    ! MCMC
    integer, parameter :: max_try = 100
    integer, parameter :: max_iter = 10000
    real(8), parameter :: Rhat_minus_1_tol = 0.01d0
    integer :: ierr, rank, nprocess
    integer :: try
    real(8), allocatable :: points(:, :) ! points(i_point, i_param)
    integer :: npoints
    ...
    real(8) :: alpha
    real(8) :: rand
    integer, parameter :: NN = max_iter
    integer :: nchains
    real(8), allocatable :: chain(:, :, :) ! chain(i_chain, i_iter, i_param)
    real(8), allocatable :: mean_chain(:, :) ! (i_chain, i_param)
    real(8), allocatable :: mean_total(:), B(:), W(:), Rhat(:) ! (i_param)
    real(8) :: Rhat_minus_1
    logical :: converged
    ...
    if (rank == 0) then
        allocate(points(max_try*nprocess*max_iter, nparams))
        allocate(points_temp(nprocess, max_iter, nparams))
        allocate(chain(max_try*nprocess, max_iter, nparams))
        allocate(mean_chain(max_try*nprocess, nparams))
        allocate(mean_total(nparams))
        allocate(B(nparams))
        allocate(W(nparams))
    ...
    if (rank == 0) then
        ...
        npoints = try*nprocess*max_iter
        
        do i = 1, nprocess
            do j = 1, max_iter
                do k = 1, nparams
                    points((try - 1)*nprocess*max_iter + (i - 1)*max_iter + j, k) &
                    = points_temp(i, j, k)
                end do
            end do
        end do
        ...
        do i = 1, nparams
            do j = 1, nchains
                do k = 1, NN
                    mean_chain(j, i) = mean_chain(j, i) + chain(j, k, i)
                end do
                mean_chain(j, i) = mean_chain(j, i) / real(NN, 8)
            end do
        end do
        ...
        do i = 1, nparams
            do j = 1, nchains
                mean_total(i) = mean_total(i) + mean_chain(j, i)
            end do
            mean_total(i) = mean_total(i) / real(nchains, 8)
        end do
        ...
        do i = 1, nparams
            do j = 1, nchains
                B(i) = B(i) + (mean_chain(j, i) - mean_total(i))**2
            end do
            B(i) = B(i) * NN / real(nchains - 1, 8)

            do j = 1, nchains
                do k = 1, NN
                    W(i) = W(i) + (chain(j, k, i) - mean_chain(j, i))**2
                end do
            end do
            W(i) = W(i) / real(nchains * (NN - 1), 8)
        end do

        do i = 1, nparams
            Rhat(i) = ((NN - 1.0d0) / NN) * W(i) + (1.0d0 + 1.0d0 / nchains) * B(i)
            Rhat(i) = Rhat(i) / W(i)
        end do
...
subroutine DLsol()
    ...
    do i = 2, n_array_z
        ...
        z_i = z_im1 + dz
        dL_i = dL_im1 + (1.0d0/6.0d0)*(k_dL_1 + 2.0d0*k_dL_2 + 2.0d0*k_dL_3 + k_dL_4)
...
function DL(z)
    ...
    call spline_out(array_z(1:n_array_z), array_dL(1:n_array_z), array_dL_buffer(1:n_array_z), &
...
! CMB_Planck18
! arXiv:1811.07425
! Central values for {R, la, obh2, h} from Planck 2018 likelihood
...
subroutine CMB_Planck2018()
    ...
    if (is_flat == .true.) then
        open (unit = 11, file = './data/CMB_Planck2018/CMB_Planck2018-flat-cov.txt', status = 'old')
    else
        open (unit = 11, file = './data/CMB_Planck2018/CMB_Planck2018-non-flat-cov.txt', status = 'old')
    end if
...
function chi2CMB()
    ...
    chi2CMB = dot_product(vecCMB(), temp)

end function chi2CMB
...
function chi2BAO()
    ...
    chi2BAO = dot_product(vecBAO(), temp)

end function chi2BAO
...
function chi2SN()
    ...
    chi2SN = A + log(E/(2.0d0 * 3.141592653589793d0)) - (B**2)/E
...
function chi2total()
    ...
    chi2total = chi2CMB() + chi2BAO() + chi2SN()
